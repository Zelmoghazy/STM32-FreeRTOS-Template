    ...
    for (;;) {
        ...
        xStatus = xQueueSendToBack(
            xQueue,   // The handle of the queue to which the data is being sent
                      // (written).
            &lValueToSend,   // A pointer to the data to be copied into the
                             // queue.
...
static void vReceiverTask(void *pvParameters) {
    ...
    for (;;) {
        ...
        if (uxQueueMessagesWaiting(xQueue) !=
            0)   // uxQueueMessagesWaiting() queries the number of items
        ...
        xStatus = xQueueReceive(
            xQueue,   // The handle of the queue from which the data is being
                      // received (read).
            &lReceivedValue,   // A pointer to the memory into which the
...
int main(void) {
    ...
    xQueue =
        xQueueCreate(5,   // The maximum number of items that the queue being
                          // created can hold at any one time.
                     sizeof(int32_t));   // The size in bytes of each data item
